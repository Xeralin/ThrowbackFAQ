...
export function Hero({ tag, corner, title, description }: HeroProps) {
  return (
    <div className="relative mb-8 min-h-[160px] overflow-hidden rounded-[10px] border border-border-brand bg-[linear-gradient(135deg,#0d0d0f_0%,#1a0810_50%,#0d0d0f_100%)] before:pointer-events-none before:absolute before:inset-0 before:z-[1] before:rounded-[inherit] before:bg-[repeating-linear-gradient(0deg,transparent,transparent_2px,rgba(0,0,0,0.06)_2px,rgba(0,0,0,0.06)_4px)] before:content-[''] after:pointer-events-none after:absolute after:inset-x-0 after:z-[3] after:h-[60px] after:animate-scan-move after:bg-[linear-gradient(transparent,rgba(192,21,42,0.025)_50%,transparent)] after:content-['']">
      ...
      <div className="relative z-[2] flex flex-col justify-center px-10 py-7 max-[48em]:px-6 max-[48em]:py-5">
      ...
      <div className="pointer-events-none absolute right-8 top-1/2 -translate-y-1/2 animate-flicker select-none font-display text-[7rem] font-bold leading-none text-[rgba(192,21,42,0.22)] max-[48em]:hidden">
        {corner}
      </div>
    </div>
  );
